...
<head>
    ...
    <?php      
        ...
        if($_GET['IDapt']!="") {  // if the Quick Search was not left blank
            $query .= " AND IDapt='$IDapt'"; // limit query to exactly 1 Apt
        }
    
        if($_GET['IDbldg']!=-1) {  // if the Quick Search was not left blank
            $query .= " AND IDbldg='$IDbldg'"; // limit query to exactly 1 Apt
        }
        ...
        if($_GET['search']!="") {  // if the user entered any text into the search box
            $search = $_GET['search'];
